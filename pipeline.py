...
class DataPipeline(Pipeline):
  ...
  def check_steps(self, verbose=True):
      ...
      for i, (desc_out, func_out) in enumerate(self.steps[:-1]):
          ...
          match = False
          type_out = f if isinstance(f:=func_out.type_out[0], tuple) else (f,)
...
class LoadRawStep(BaseStep):
  ...
  **kwargs : dict
  ...
  def _transform(self, X):
      ...
      if fname.suffix == '.fif':
          raw = mne.io.read_raw_fif(fname, **params)
      elif fname.suffix == '.vhdr':
          raw = mne.io.read_raw_brainvision(fname, **params)
      else:
...
class FilterStep(BaseStep):
  ...
  def __init__(
      ...
      self.type_in = (mne.filter.FilterMixin,), 'Any MNE object that implements .filter'
...
class ResampleStep(BaseStep):
  ...
  def __init__(
      ...
      *,
      ...
      self.type_in = (mne.io.BaseRaw,), 'Any MNE Raw object that can be resampled'
  ...
  def _transform(self, raw):
      ...
      if raw_sfreq_rounded == self.sfreq:
          logging.info('Sample frequency is already what is requested, skipping ResampleStep')
          return raw
...
class EpochingStep(BaseStep):
  ...
  def _transform(
      ...
      # Create epochs from the raw data
      epochs = mne.Epochs(raw, events, **self.get_params())
...
class NormalizationStep(BaseStep):
  ...
  def __init__(self, norm_func, axis=None, picks=None):
      self.type_in = (np.ndarray, mne.io.BaseRaw, mne.epochs.BaseEpochs), 'Any MNE obj or array'
      self.type_out = (np.ndarray, mne.io.BaseRaw, mne.epochs.BaseEpochs), 'Any MNE obj or array'
      ...
      if isinstance(obj, (mne.io.BaseRaw, mne.epochs.BaseEpochs)):
          ...
          if isinstance(obj, mne.io.BaseRaw):
              ...
              obj_normalized = mne.EpochsArray(
                  data_normalized, obj.info, events=obj.events, event_id=obj.event_id, tmin=obj.tmin
              )
...
class ToArrayStep(BaseStep):
  ...
  def __init__(self, X=True, y=True, picks=None):
      self.type_in = (mne.io.BaseRaw, mne.epochs.BaseEpochs), 'Any MNE object'
  ...
  def _transform(self, obj):
      ...
      data = []
      if self.X:
          data_x = obj.get_data(picks=self.picks)
          data.append(data_x)
      if self.y:
...
class ICAStep(BaseStep):
    ...
    def _transform(self, raw):
        ...
        ica = mne.preprocessing.ICA(n_components=self.n_components, method=self.method, random_state=self.random_state, max_iter=self.max_iter, verbose=self.verbose)
        # Select channels to include (e.g., MEG/EEG channels)
        picks = raw.pick(['eeg', 'meg'], exclude='bads')
        # Fit ICA
        ica.fit(raw, picks=picks, decim=self.decim, reject_by_annotation=self.reject_by_annotation, tstep=self.tstep)
        # Find EOG artifacts
...
class StratifyStep(BaseStep):
  def __init__(self, strategy='undersample', random_state=None, verbose=None):
      ...
      self.type_in = (mne.epochs.BaseEpochs,), 'Epochs object to be stratified'
  ...
  def _transform(self, epochs):
      ...
      if self.strategy == 'undersample':
          ...
          for event in event_ids:
              ...
              indices = rng.choice(len(these_epochs), size=min_count, replace=False)
          ...
          raise ValueError(f"Unknown strategy '{self.strategy}'. Use 'undersample' or 'oversample'.")
